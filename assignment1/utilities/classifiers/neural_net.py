...
class TwoLayerNet(object):
	...
	def loss(self, X, y=None, reg=0.0):
		...
		# ====================================================================== #
		# Compute the forward pass, computing the class scores for the input.    #
		...
		h = f(np.dot(X, W1) + b1) # first hidden layer activation
		scores = np.dot(h, W2) + b2 # output layer
		...
		# exponentiate log probs => unormalized probs
		unormalized_probs = np.exp(scores)
		# get sum of unormalized probs
		denum = np.sum(unormalized_probs, axis=1, keepdims=True)
		# divide by sum => normalized probs
		probs = unormalized_probs / denum

		# select from each row of probs using indices in y
		correct_log_probs = -np.log(probs[np.arange(N), y])
		# now compute cross-entropy loss
		data_loss = np.sum(correct_log_probs / N)

		# L2 regularization loss
		lw1 = np.sum(W1 * W1) # W1 loss
		lw2 = np.sum(W2 * W2) # W2 loss
		reg_loss = 0.5 * reg * (lw1 + lw2) 
		...
		# finally, backpropagate into W1 and b1
		grads['W1'] = np.dot(X.T, dh)
		grads['b1'] = np.sum(dh, axis=0, keepdims=True)

		# and add regularization gradient contribution
		grads['W2'] += reg * W2
		grads['W1'] += reg * W1
	...
	def train(self, X, y, X_val, y_val, learning_rate=1e-3, learning_rate_decay=0.95, reg=1e-5, num_iters=100, batch_size=200, verbose=False):
		...
		for it in range(num_iters):
			...
			for parameter in self.params:
				...
				grads[parameter].shape = self.params[parameter].shape
				# parameter update
	...
	def predict(self, X):
		...
		# output layer
		log_probs = np.dot(h, self.params['W2']) + self.params['b2']

		# compute normalized probs
		unormalized_probs = np.exp(log_probs)
		# get sum of unormalized probs
		denum = np.sum(unormalized_probs, axis=1, keepdims=True)
		# compute scores == normalized probs
		scores = unormalized_probs / denum

		# for each row, grab index of highest score
